	...
	double				eps)			// the error bound
{
	...
	var.ANNkdDim = dim;						// copy arguments to static equivs
	var.ANNkdQ = q;
	var.ANNkdPts = pts;
	...
	var.ANNkdMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating op count

	var.ANNkdPointMK = new ANNmin_k(k);		// create set for closest k points
										// search starting at the root
	root->ann_search(annBoxDistance(q, bnd_box_lo, bnd_box_hi, dim), var);

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = var.ANNkdPointMK->ith_smallest_key(i);
		nn_idx[i] = var.ANNkdPointMK->ith_smallest_info(i);
	}
	delete var.ANNkdPointMK;				// deallocate closest point set
}
...
void ANNkd_split::ann_search(ANNdist box_dist, ANNVariables & var)
{
										// check dist calc term condition
	if (var.ANNmaxPtsVisited != 0 && var.ANNptsVisited > var.ANNmaxPtsVisited) return;

										// distance to cutting plane
	ANNcoord cut_diff = var.ANNkdQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		child[ANN_LO]->ann_search(box_dist, var);// visit closer child first

		ANNcoord box_diff = cd_bnds[ANN_LO] - var.ANNkdQ[cut_dim];
		...
		if (box_dist * var.ANNkdMaxErr < var.ANNkdPointMK->max_key())
			child[ANN_HI]->ann_search(box_dist, var);

	}
	else {								// right of cutting plane
		child[ANN_HI]->ann_search(box_dist, var);// visit closer child first

		ANNcoord box_diff = var.ANNkdQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		box_dist = (ANNdist) ANN_SUM(box_dist,
				ANN_DIFF(ANN_POW(box_diff), ANN_POW(cut_diff)));
										...
										// visit further child if close enough
		if (box_dist * var.ANNkdMaxErr < var.ANNkdPointMK->max_key())
			child[ANN_LO]->ann_search(box_dist, var);
...
void ANNkd_leaf::ann_search(ANNdist box_dist, ANNVariables & var)
{
	...
	min_dist = var.ANNkdPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = var.ANNkdPts[bkt[i]];			// first coord of next data point
		qq = var.ANNkdQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < var.ANNkdDim; d++) {
		...
		if (d >= var.ANNkdDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			var.ANNkdPointMK->insert(dist, bkt[i]);
			min_dist = var.ANNkdPointMK->max_key();
		}
	}
