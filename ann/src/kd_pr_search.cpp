	...
	double				eps)			// error bound (ignored)
{
	...
	var.ANNprMaxErr = ANN_POW(1.0 + eps);
	ANN_FLOP(2)							// increment floating ops

	var.ANNprDim = dim;						// copy arguments to static equivs
	var.ANNprQ = q;
	var.ANNprPts = pts;
	var.ANNptsVisited = 0;					// initialize count of points visited
	var.ANNmaxPtsVisited = ANNmaxPtsVisited;

	var.ANNprPointMK = new ANNmin_k(k);		// create set for closest k points

										// distance to root box
	ANNdist box_dist = annBoxDistance(q,
				bnd_box_lo, bnd_box_hi, dim);

	var.ANNprBoxPQ = new ANNpr_queue(n_pts);// create priority queue for boxes
	var.ANNprBoxPQ->insert(box_dist, root); // insert root in priority queue

	while (var.ANNprBoxPQ->non_empty() &&
		(!(var.ANNmaxPtsVisited != 0 && var.ANNptsVisited > var.ANNmaxPtsVisited))) {
		ANNkd_ptr np;					// next box from prior queue

										// extract closest box from queue
		var.ANNprBoxPQ->extr_min(box_dist, (void *&) np);

		ANN_FLOP(2)						// increment floating ops
		if (box_dist*var.ANNprMaxErr >= var.ANNprPointMK->max_key())
			break;

		np->ann_pri_search(box_dist, var);	// search this subtree.
	}

	for (int i = 0; i < k; i++) {		// extract the k-th closest points
		dd[i] = var.ANNprPointMK->ith_smallest_key(i);
		nn_idx[i] = var.ANNprPointMK->ith_smallest_info(i);
	}

	delete var.ANNprPointMK;				// deallocate closest point set
	delete var.ANNprBoxPQ;					// deallocate priority queue
}
...
void ANNkd_split::ann_pri_search(ANNdist box_dist, ANNVariables & var)
{
	ANNdist new_dist;					// distance to child visited later
										// distance to cutting plane
	ANNcoord cut_diff = var.ANNprQ[cut_dim] - cut_val;

	if (cut_diff < 0) {					// left of cutting plane
		ANNcoord box_diff = cd_bnds[ANN_LO] - var.ANNprQ[cut_dim];
		...
		if (child[ANN_HI] != KD_TRIVIAL)// enqueue if not trivial
			var.ANNprBoxPQ->insert(new_dist, child[ANN_HI]);
										// continue with closer child
		child[ANN_LO]->ann_pri_search(box_dist, var);
	}
	else {								// right of cutting plane
		ANNcoord box_diff = var.ANNprQ[cut_dim] - cd_bnds[ANN_HI];
		if (box_diff < 0)				// within bounds - ignore
			box_diff = 0;
										// distance to further box
		new_dist = (ANNdist) ANN_SUM(box_dist,
		...
		if (child[ANN_LO] != KD_TRIVIAL)// enqueue if not trivial
			var.ANNprBoxPQ->insert(new_dist, child[ANN_LO]);
										// continue with closer child
		child[ANN_HI]->ann_pri_search(box_dist, var);
	}
...
void ANNkd_leaf::ann_pri_search(ANNdist box_dist, ANNVariables & var)
{
	...
	min_dist = var.ANNprPointMK->max_key(); // k-th smallest distance so far

	for (int i = 0; i < n_pts; i++) {	// check points in bucket

		pp = var.ANNprPts[bkt[i]];			// first coord of next data point
		qq = var.ANNprQ;					// first coord of query point
		dist = 0;

		for(d = 0; d < var.ANNprDim; d++) {
		...
		if (d >= var.ANNprDim &&					// among the k best?
		   (ANN_ALLOW_SELF_MATCH || dist!=0)) { // and no self-match problem
												// add it to the list
			var.ANNprPointMK->insert(dist, bkt[i]);
			min_dist = var.ANNprPointMK->max_key();
		}
	}
	ANN_LEAF(1)							// one more leaf node visited
	ANN_PTS(n_pts)						// increment points visited
	var.ANNptsVisited += n_pts;				// increment number of points visited
}
