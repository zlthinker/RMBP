...
void ANNbd_shrink::ann_search(ANNdist box_dist, ANNVariables & var)
{
	...
	for (int i = 0; i < n_bnds; i++) {			// is query point in the box?
		if (bnds[i].out(var.ANNkdQ)) {				// outside this bounding side?
												// add to inner distance
			inner_dist = (ANNdist) ANN_SUM(inner_dist, bnds[i].dist(var.ANNkdQ));
		}
	}
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_search(inner_dist, var);	// search inner child first
		child[ANN_OUT]->ann_search(box_dist, var);	// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_search(box_dist, var);	// search outer child first
		child[ANN_IN]->ann_search(inner_dist, var);	// ...then outer child
	}
